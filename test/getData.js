...
function defaultChart(data) {
    ...
    nv.addGraph(function() {
        ...
        chart.yAxis
        .tickFormat(d3.format(decimal));
        chart.forceY(forceY);
		chart.yDomain(yDomain);
        d3.select('#'+divname).datum(data.series).transition().duration(500).call(chart);
  ...
  nv.addGraph(function() {
    ...
    var chart = nv.models.linePlusBarChart()
      ...
      chart.xAxis
          ...
          .tickFormat(function(d) { return d3.time.format(tFormat)(new Date(d)) });
      chart.y1Axis
          .tickFormat(d3.format(decimal1));
      chart.y2Axis
          .tickFormat(function(d) { return d3.format(decimal2)(d) });
	...
	nodeFile = this.get("nodeFileLocation");
      ...
      if (this.get("loadLocal") && this.get("data")) {
        ...
        if (preFilter.is) {
           ...
           */
          console.log('No preFilter, default load');
          self.progress('start');
